...
export class Router {
  ...
  constructor({ mode, root, el, prefix, routes }) {
    this.setMode(mode);
    this.setRoot(root);
    ...
    if (!this.hasRootElm(el)) {
      console.error(
        `${debugStr} couldn't find a value for ${quotes(
          "el property"
        )} @ router instance. \nmake sure it has the right selector e.g. el:"#app" \nand ${quotes(
          "Root Element"
        )} exists in dom e.g. <div id="app"></div>`
      );
    } else if (!this.hasRoutesArr(routes)) {
      console.error(
        `${debugStr} couldn't find ${quotes("routes property")} @ router instance or it's empty. \nmake sure that ${quotes(
          "routes property"
  ...
  hasRoutesArr(routes) {
    if (!!routes && Array.isArray(routes) && routes.length) {
      routes.forEach(route => this.add(route.path, route.comp));
      return true;
    }
    return false;
  }
  ...
  setPrefix(prefix) {
    if (!!prefix) {
      ...
      this.prefix = null;
      console.warn(
        `${debugStr} couldn't find ${quotes("prefix property")} @ router instance. \nfalling back to default component tag e.g. <ComponentName/>`
      );
    }
  }
  ...
  setMode(mode) {
    if (!!mode) {
      ...
      console.warn(
        `${debugStr} couldn't find ${quotes("mode property")} @ router instance. \nfalling back to available default mode: ${quotes(this.mode)}`
      );
    }
  }

  isDev() {
    return ["localhost", "127.0.0.1"].includes(location.hostname) ? true : false;
  }
  ...
  inject(View) {
    if (!View) {
      ...
      try {
        const label = this.isDev() ? this.label(entry, compsArr) : { entry, compsArr }; // labeling components in dev mode
        ...
        // devtool
        if (this.isDev()) import("../devtool").then(module => new module.default(this.appMarkup));
      } catch (err) {
        console.error(err);
      }
    }
  ...
  label(root, arr) {
    ...
    compsArr.forEach(comp => {
      const name = comp && comp.child && comp.child.data && comp.child.data.name ? comp.child.data.name : "null";
      if (comp && comp.child && comp.child.data && comp.child.data.template) {
        // clean
        ...
        if (Array.isArray(compWrapper) && compWrapper.length) {
          comp.child.data.template = comp.child.data.template.replace(
            compWrapper[0],
            compWrapper[0].replace(greaterCharRegex, ` pk-component="${name}">`)
          );
        }
      }
    });
