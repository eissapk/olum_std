    ...
    listen() {
      global.on("popstate", () => {
        ...
        const route = this.routes.filter(item => {
          const path = item.path.lower().trim();
          if (current === this.root + "index.html") {
        ...
        if (route.length) {
          route[0].cb(); // invokes inject(view)
          this.active(route[0]);
        } else {
          if (!!this.err) {
            err = this.routes.find(item => item.path.lower().trim() === this.err);
            err.cb();
          } else {
            // console.error(`${debugStr} ${quotes(`${current}`)} route is not defined!`);
      ...
      this.views.forEach(View => {
        ...
        compsArr.forEach(comp => {
          if (hasProp(comp.child, "data")) {
            ...
            styles.push(compStyle); // push children styles
          }
        });
      });
      document.body.insertAdjacentHTML("afterbegin", `<style>${styles.join("\n")}</style>`); // add it to the beginning of the body to override global styling in the head
    }
