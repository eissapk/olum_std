...
export class Router {
  ...
  inject(View) {
    if (!View) {
      ...
      try {
        const label = this.isDev() ? this.label(entry, compsArr) : {
          entry,
          compsArr
  ...
  label(root, arr) {
    ...
    if (entry.template) {
      ...
      if (Array.isArray(compWrapper) && compWrapper.length) {
        entry.template = entry.template.replace(compWrapper[0], compWrapper[0].replace(greaterCharRegex, ` pk-component="${name}">`));
      }
    }
