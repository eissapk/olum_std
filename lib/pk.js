...
export class OnInit {
  ...
  scope(data) {
    ...
    if (compWrapper.length) {
      data.template = data.template.replace(compWrapper[0], compWrapper[0].replace(this.greaterCharRegex, ` pk-${data.name.toLowerCase()}-component>`));
    }
...
// todo fix duplicated events , check event listeners tab 
export class Service {

  constructor(name) {
    this.serviceEvent = new Event(name);
    this.event = name;
  }
