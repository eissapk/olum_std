...
/**
 * `e.g. debug("test", "trace")`
 *
 * `Tip:` spreads any passed array, err = error
 *
 * @param {*} args accepts all types
 * @param {*} level (optional) equal `log` by default, accept all console methods e.g. warn, error and so on
 */
...
/**
 * `e.g. $("#header") => returns one element`
 * 
 * `e.g. $("#header", true) => returns array`
 * 
 * `e.g. $(elm).get("li") => returns one li inside elm tag`
 * 
 * `e.g. $(elm).get("li", true) => returns array of li inside elm tag`
 * 
 * @param {*} target can be class, id, tag or Element
 * @param {*} level (optional) boolean if omited it returns one element otherwise (true) returns array
 */
export const $ = (target, level, el = document) => {
  ...
  if (typeof target == "string") {
    ...
    if (elms.length) {
      ...
      console.error(`${debugStr}${quotes(target)} doesn't exist.`);
    }
  } else {
    // element as reference
    elms = target;
    if (!Array.isArray(elms)) elms.get = (t, l) => $(t, l, (el = elms));
    return elms;
  }
};

/**
 * `e.g. elm.on(event, cb, propagation)`
 *
 * @param {*} event the native javascript events such as `click`, `keyup` and so on
