...
export class Router {
  ...
  // inject passed View "component" to dom
  inject(View) {
    const root = document.querySelector(this.el);

    if (!root) {
      debug(`Root element is not found!`);
    } else if (!View || typeof View != "function") {
      debug(`Component object doesn't have props: (template, style, sha, cb)`);
    } else {
      const view = new View();
      const comp = view.init();

      console.groupCollapsed(view.constructor.name + " View");
      debug(comp);
      console.groupEnd();

      const arr = Object.keys(comp);
      if (Array.isArray(arr) && arr.length && comp.template && comp.style && comp.cb && comp.sha) {
        // inject html
        ...
        // run js
        const arr_ = Object.keys(comp.cb);
        if (Array.isArray(arr_) && arr_.length) {
...
export class OnInit {
  ...
  init({ template, style, components = {}, render, scoped = true }) {
    let ob;
    if (scoped) {
      ob = this.scoped(template, style);
    } else {
      ob = { template, style };
    }
    ...
    if (typeof components != "undefined" && Array.isArray(arr) && arr.length) {
      for (let key in components) {
        num++;
        const pattern = new RegExp("<" + key + "+(>|.*?[^?]>)", "gi"); // detect components e.g. <Header/>
        const instance = new components[key]().init();
