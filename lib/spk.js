...
export class OnInit {
  ...
  init({
    ...
    let ob = this.scope(components, template, style, scoped);
    ob = this.merge(components, ob, render);
    return ob;
  }

  scope(components, template, style, scoped) {
    const attrRegex = /data-spk=\"([^"]*)\"|data-spk/gi; // detect attribute ----> data-spk="" or data-spk
    const openTagregex = /<[a-z]+(>|.*?[^?]>)/gi; // detect the opening tag ----> <tag> or <tag/>
    const openTagregex_ = /<([^\/>]+)>/gi; // detect the opening tag ----> <tag>
    const selfCloseRegex = /\/\>/gi; // detect self-closing tag -----> "/>"
    const closeCharRegex = /\>/gi; // detect greater than char -----> ">"
    const styleRegex = /\s*\{/g; // detect ----> "{" 
    const sha = new Date().getTime();

    if (scoped === true) {

      //? HTML
      template = template.replace(attrRegex, ""); // clean attributes
      let openTagArr = template.match(openTagregex); // extract opening tags

      // 1- replace self-closing tag <tag/>
      if (openTagArr.length) {
        openTagArr.forEach(item => template = template.replace(item, item.replace(selfCloseRegex, ` data-spk="${sha}" />`)));
      }

      // // 2- replace opening tag <tag>
      openTagArr = template.match(openTagregex_);
      if (openTagArr.length) {
        openTagArr.forEach(item => template = template.replace(item, item.replace(closeCharRegex, ` data-spk="${sha}" >`)));
      }
      ...
      if (Array.isArray(compKeys) && compKeys.length) {
        for (let key in components) {
          const compRegex = new RegExp(`<(.*${key}.*)\\/>`, "gi");
          const extractedCompArr = template.match(compRegex);
          if (extractedCompArr.length) {
            extractedCompArr.forEach(item => template = template.replace(item, item.replace(attrRegex, "")));
          }
        }
      }

      //? CSS
      style = style.replace(styleRegex, `[data-spk="${sha}"]{`);
  ...
  merge(components, ob, render) {
    ...
    if (Array.isArray(compKeys) && compKeys.length) {
      for (let key in components) {
        ...
        const ClassComp = components[key];
        const initialize = new ClassComp().init();
